...
from stock_analyser.tools.yfinance_tools import get_fundamentals, get_financials
from stock_analyser.tools.source_urls import build_url_block

_TOOLS = [get_fundamentals, get_financials]
...
_PROMPT = """You are a fundamental analysis expert. Your job is to analyse the stock {ticker} \
and produce a buy/hold/sell signal based on its financial health and valuation.

You have two tools available:
- get_fundamentals: fetches valuation ratios, margins, ROE, growth rates — call this first
- get_financials: fetches income statement, balance sheet, cash flow — call this for deeper analysis
...
the tools are authoritative. If the tools return data, the stock is actively traded and \
you must analyse it from those numbers. Never skip tool calls based on what you think you know.

Call both tools to gather all the data you need. Once you have the data, return your analysis \
...
You MUST call the tools before returning the JSON. Return ONLY the JSON object — no markdown, \
no explanation, no code fences.

BULLET COUNT RULES:
- Return 1–5 bullets based on data richness — never pad to hit a fixed number
- 1–2 bullets: sparse data or most fields returned None
...
_QA_PROMPT = """You are a fundamental analysis expert. A user is asking a follow-up question about \
a stock's fundamentals. Use the available tools to fetch any data you need and answer directly.

Ticker: {ticker}
User question: {question}

Tools available:
- get_fundamentals: valuation ratios, margins, growth rates, dividends
- get_financials: income statement, balance sheet, cash flow

Call whichever tool(s) are needed to answer the question. Then respond with ONLY a JSON object:
{{
...
Writing rules:
- Plain English — explain any financial terms you use
- Be specific with numbers if available
- Honest about what the data does or doesn't show
...
def fundamentals_agent(state: GraphState) -> dict:
    ticker = state["ticker"]
    try:
        ...
        return {"fundamentals_signal": signal}
    except Exception as e:
        return {"fundamentals_signal": AgentSignal(
...
def fundamentals_qa_agent(state: GraphState) -> dict:
    ...
    try:
        ...
        return {"agent_answers": {"fundamentals": answer_obj}}
    except Exception as e:
        return {"agent_answers": {"fundamentals": AgentAnswer(
            answer=f"Fundamentals data unavailable: {e}",
            sources=[],
            confidence=0.0,
        )}}
    finally:
